...
이 플러그인(armarium)의 스킬 파일에서 절대 금지 규칙을 표시할 때 쓰는 태그는 `<FORBIDDEN>` 하나다.

- `<FORBIDDEN>` — 절대 실행하지 않는다. 맥락과 무관하게 차단한다.

수행 규칙에는 태그를 쓰지 않는다. 반드시 해야 할 일은 실행 순서의 단계로 적는다 -- 빠뜨리면 산출물에 드러나므로 별도 표시가 필요 없다. 금지는 절차에 자리가 없고 어겨도 그 자리에서 드러나지 않아 태그가 유일한 표시 수단이다.

붙일 조건, 블록 입자, 총량 등 작성 규격은 `skill-writing` 을 따른다.
