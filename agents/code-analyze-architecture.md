...
## 입력
- `<브랜치 디렉토리>/facet/skeleton.md` (모듈 목록/진입 표면 -- 단일 출처).
- 분석 대상 경로.
...
- **진입점** -- 실행 시작점.
- **진입 표면 -> 핸들러 매핑** -- 외부 트리거가 코어의 어느 동작으로 가나(진입표면은 골격에서). 예: CLI 서브커맨드->유스케이스 = HTTP 라우트->핸들러 = 메시지 토픽->컨슈머 = 공개 API->구현. 코드에 있는 형태로 인스턴스화하고, **매핑 대상(핸들러/구현 노드)을 추정 라벨이 아니라 실제 노드로 명시하고 구조 노드로 링크**한다.
- **포트 <- 구현 관계엣지** -- 경계 인터페이스와 그 구현(어댑터)의 짝. 대상 구현 노드를 명시하고 링크한다.
