...
2. 검증 대상 파일 목록을 만든다. 대상 스킬 디렉토리 안의 모든 마크다운 파일에 더해, 대상 스킬의 실행 순서에서 호출하는 에이전트를 읽어 플러그인 루트 `agents/` 의 해당 정의 파일과, 본문이 Read 하도록 지시하는 공유 참조 파일(`references/` 등)을 포함한다.
