...
## 입력
- `<브랜치 디렉토리>/facet/`: 가로지르는 문서 7종(architecture/flow/data-contract/test/externals/invariants/conventions) + 구조 트리 + index.
- `<브랜치 디렉토리>/facet/flow.diagrams.md` (있으면): 플로우 시퀀스 다이어그램(facet 에 지속됨).

## 엮는 범위 (중요)
- **가로지르는 7종 + 구조 개요**만 내러티브로 엮는다. 구조 개요 = 모듈/레이어 수준의 큰 그림(어떤 모듈이 무엇을 담나).
...
- 구성: 인트로(이 시스템이 무엇인지 as-built) -> 아키텍처/구조 개요 -> 플로우 -> 데이터 계약 -> 불변식 -> 외부 의존 -> 테스트 현황 -> 관례(네이밍/패턴/함정 + 분석 스킵 기록).
