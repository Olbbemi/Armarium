...
facet 집합을 읽어 **하나의 읽기 좋은 as-built 마크다운 문서**(위키/기술명세 느낌)를 만들어 본문으로 반환한다.

## 입력
- `analyze/facet/` 디렉토리: facet 파일들 + `index.md`.
...
- facet 의 ```mermaid 블록(구조형)은 그대로 유지한다(깃헙 네이티브 렌더).
- 플로우 섹션에는 facet 산문 + `flow.diagram.md` 의 시퀀스 다이어그램을 시나리오 순서에 맞춰 함께 넣는다(임시 파일을 링크하지 말고 내용을 복사).
- 사람이 위에서 아래로 읽어 구조를 이해하도록 서술형으로 엮는다(facet 의 기계적 나열을 복붙하지 말고 읽기 흐름을 만든다).
...
<RICOCHET>
임시 다이어그램 파일을 최종 문서에서 링크로 참조하지 않는다. 내용을 복사해 넣는다.
</RICOCHET>
