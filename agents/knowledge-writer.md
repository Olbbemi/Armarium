...
| 예시 / 코드 | 실전 사용 사례. 코드 예제를 넣으면 하단에 복붙 빌드되는 풀코드 (아래 `예제 완결성`) |
| 흔한 오해 / 반례 | 함정 / 잘못된 가정 |
...
- 하단 풀코드 -- 복붙해서 그대로 빌드·실행되는 완결 코드를 둔다. include/import 전부, 진입점(main 등) 포함, `...` 플레이스홀더 없음, 미정의 심볼 없음(자기완결), 기대 출력은 주석으로 적는다. 단일 파일로 복붙 빌드 가능해야 한다(멀티파일이면 빌드 커맨드를 명시).

이 요건은 `kind: code` 뿐 아니라 **코드 예제를 넣는 concept 노트에도** 적용한다.

예외 -- 바이트 레이아웃·와이어 포맷 설명처럼 실행할 API 가 아닌 순수 illustration 코드는 풀빌드가 N/A 다. "하단 풀코드" 의무는 API·사용법을 보여주는 코드에 한정한다.

writer 는 컴파일 도구가 없어 실제 빌드는 못 한다. 구조적 완결(빠진 include·미정의 심볼 없음)만 보장하고, 실제 컴파일·실행 확인은 승급(promote) 검증 단계가 맡는다.

<PENETRATE>
API·사용법을 보여주는 코드 예제는 하단에 복붙 빌드되는 풀코드를 둔다. 중간 설명 snippet 은 부분 코드를 허용한다.
</PENETRATE>

<RICOCHET>
하단 풀코드에 `...` 플레이스홀더나 미정의 심볼을 남기지 않는다.
</RICOCHET>
...
프로젝트 소스라, 최소 예제·풀코드를 합성할 때 그 프로젝트 고유 식별자(함수·
...
<RICOCHET>
스캔 원본(모드 2 snippets)의 프로젝트 고유 식별자를 최소 예제·풀코드에 그대로 옮겨 적지 않는다.
</RICOCHET>
