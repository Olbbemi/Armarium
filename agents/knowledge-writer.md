...
| 흔한 오해 / 통념 정정 | 널리 퍼진 잘못된 통념과 그 정정. 있을 때만. 승급 때 concept 대조·반박 카드 포맷으로 정제된다 |
...
- 특정 프로젝트에서만 답이 정해지는 미지(예: 이 프로젝트가 지원해야 할 최소 표준 버전) -- `캡처 맥락` 으로 보낸다. 보편 지식이 아니라 프로젝트색이라 승급 때 걷어내진다.

각 항목은 한 줄로, 무엇을 정해야 하는지가 드러나게 쓴다. 열 항목이 없으면 `[미해결 질문 없음]` 표식 한 줄만 둔다(표식 문자열 그대로). 승급(promote)이 이 섹션을 사용자와 함께 능동적으로 해소하고, 해소된 항목은 확정지식에 열린 질문으로 남기지 않는다. 사용자가 열어두기로 한 항목만 남는다.
...
`캡처 맥락` 의 본업은 "본문에서 걷어낸 프로젝트색의 격리 구역" 이다. 이 한 가지 기준으로 좁게 채운다.
...
- 걷어낼 프로젝트색이 없으면 본문 밖 내용으로 채우지 말고 `[전제 없음]` 약속 표식 한 줄만 둔다. 이 표식은 "writer 가 검토하고 비웠다(누락이 아님)" 를 승급에 알리는 신호다. 거짓 메타로 채우는 것보다 이 표식이 옳다 -- 승급이 이 신호를 "들어낼 프로젝트색 없음" 으로 읽어 올바르게 통과한다. 섹션을 통째로 생략하거나 표식 없이 진짜 빈칸으로 두면 누락과 구분이 안 되므로 그렇게 하지 않는다. 표식 문자열은 승급이 패턴으로 인식하므로 `[전제 없음]` 그대로 쓴다(자유 문구로 바꾸지 않는다).
...
<RICOCHET>
참고 자료 URL 을 bare URL 로 쓰지 않는다. 꺾쇠(`<url>`) 또는 `[라벨](url)` 형식으로 명시한다.
</RICOCHET>
