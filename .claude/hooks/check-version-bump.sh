#!/usr/bin/env bash
# PreToolUse(Bash) 훅: armarium 저장소에서 git push 시
# .claude-plugin/plugin.json 의 version 이 직전 푸시(upstream)와 같으면
# 사용자에게 "버전을 올릴지 / 그대로 진행할지" 확인을 요청한다.
#
# version 을 안 올리고 push 하면 /plugin marketplace update 시 변경이
# 반영되지 않으므로(무의미한 update), 그 사고를 push 직전에 차단한다.
set -uo pipefail
