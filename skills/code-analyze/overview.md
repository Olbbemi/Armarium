...
analyze 산출물은 코드의 거울(파생물)이라, 코드가 바뀌면 옛 분석은 현재 코드와 안 맞는 stale 자료가 된다. 기본은 **덮어쓰기**다 -- 재분석하면 기존 `analyze/` 를 갱신해 항상 현재 코드에 맞는 거울 하나만 둔다. 과거 시점 분석이 필요하면 그 시점 코드(git)를 다시 분석해 재현한다.

예외는 프로젝트가 **버저닝 단계**일 때다. 현재 git 브랜치가 릴리스 브랜치(`release/*` 등)면 그 버전의 as-built 를 스냅샷으로 남기기 위해 덮어쓰지 않고 `analyze_<버전>/` 로 누적 보존한다(버전은 릴리스 브랜치 / 태그에서 딴다 -- 예: `release/1.2.0` -> `analyze_1.2.0/`). 릴리스는 의미 있는 동결점이라 그 시점 분석을 남길 가치가 있기 때문이다.
...
2. 출력 루트를 정한다 -- 현재 git 브랜치를 확인해, 릴리스 버저닝 브랜치(`release/*` 등)면 `analyze_<버전>/`, 아니면 `analyze/`(기존이 있으면 덮어쓰기). 그 아래 `{facet,html,markdown,.tmp}/` 를 메인이 Bash 로 생성한다(`mkdir -p`). 재분석 시 갱신 정책은 "출력 구조" 참조.
