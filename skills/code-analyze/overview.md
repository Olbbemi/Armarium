...
**언어 일반화.** 범용 에이전트는 역할어로만 말하고(도메인/스택 무관), 언어 고유 규칙(노드 단위·한정자 어휘 등)은 에이전트가 아니라 `skills/code-analyze/languages/<언어>.md` 에 둔다 -- 메인이 판정한 언어의 규칙 파일을 각 에이전트가 로드해 적용한다. 언어 추가 = 규칙 파일 하나. 예외는 **호출그래프**뿐 -- 도구축이 언어마다 통째로 달라 규칙 조각이 아니라 언어별 에이전트(`callgraph-<언어>`)다.
...
<PENETRATE>
진행도 task 는 메인만 만든다 -- 에이전트 호출은 메인 쪽 "위임 완료" task 하나로 표시하고, 에이전트 내부엔 task 를 두지 않는다.
</PENETRATE>
