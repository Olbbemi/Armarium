# code-analyze

코드를 분석해 **as-built 코드 지도**(실제로 짜인 구조)를 만들고, 청중별 산출물을 생성하는 독립 스킬.

## 무엇을 하나

지정한 코드를 분석해 그 결과에서 세 청중용 산출물을 생성한다(분석은 한 번만 돈다). 핵심 산출은 **facet** -- "읽기 좋은 요약"이 아니라 Claude 가 **리팩토링 + 유닛테스트(TDD 부족분 보강)** 에 바로 쓰는 다관점·최대 상세 레퍼런스다. 사람용 서술은 별도 산출물(markdown/html)이 맡는다.

facet 은 두 모양으로 나뉜다.

- **구조 트리** -- 소스 트리를 미러링한 코드-major 문서들. 노드 = 소스 파일 1개(선언/정의 분리 언어면 같은 단위로 묶음). 그 파일이 담은 타입/함수의 전체 시그니처 + 속성(역할/불변식/예외).
- **가로지르는 관점 6종** -- 파일 하나에 안 붙는 전체-뷰 문서.

| 관점 | 보는 것 |
|------|---------|
| architecture | 경계/의존 지도 -- 의존 방향, 진입점, 진입표면->핸들러 매핑, 포트<-구현 |
| flow | 트리거 + 생명주기에 앵커된 end-to-end 경로 망라(핵심만) |
| data-contract | 영속/설정 스키마 = 데이터 계약(있으면) |
| test | 테스트 인벤토리 + 커버리지 공백 |
| externals | 단일 소스 의존성 레지스트리(라이브러리·버전·경계) |
| invariants | 전체 관통 규약/불변식 단일 소스 -- TDD 테스트의 핵심 원천 |

**단일 소스 + 링크 원칙**: 개수·외부 의존·전역 불변식·노드 상세는 각각 한 곳만 소유하고, 나머지 문서는 링크만 한다(재서술 = 드리프트).

## 언제 쓰나

`/code-analyze` 슬래시 명령으로 **명시적으로 호출**한 경우에만 동작한다. 자동 호출 대상이 아니며 "이 코드 분석해줘" 같은 자연어로는 발동하지 않는다.

## 산출물 (실행 범위에 따라)

베이스 산출은 **facet** 이고, 시각화(html/markdown)와 검증은 착수 전 **실행 범위 게이트**에서 옵트인한다 -- facet 만 만들고 멈출 수도 있다(아래 "동작 흐름" 참조). **분석 대상 디렉토리 아래에** `analyze/` 를 만들고, 그 밑에 **브랜치별 디렉토리**(= 브랜치 디렉토리, 이름은 분석 시점 git 브랜치 이름의 `/` 를 `_` 로 치환)를 두어 종류별 하위 디렉토리에 저장한다(레포 루트가 아니라 분석한 그 디렉토리 밑).

```
<분석 대상 디렉토리>/
  analyze/
    <브랜치 슬러그>/   브랜치 디렉토리 = 브랜치 하나의 분석 한 벌 (예: feature_<이름>, release_<버전>)
      facet/       Claude 층 -- 구조 트리(소스 미러) + 가로지르는 6종 + skeleton + index
      html/        인터랙티브 HTML (사람용, Mermaid 클라이언트 렌더)
      markdown/    as-built 마크다운 문서 (사람용 -- 가로지르는 5종 + 구조 개요를 엮은 내러티브)
      .tmp/        렌더 중간 산출물. 렌더 후 삭제
```

생성만 한다. 커밋·이동(특히 HTML 을 GitHub Pages 디렉토리로)·푸시는 사용자가 결과를 보고 직접 한다.

### 재분석 시 갱신

- 보관 단위는 브랜치별 한 벌 -- 브랜치마다 자기 브랜치 디렉토리(`analyze/<브랜치 슬러그>/`)에 그 브랜치 코드를 반영한 한 벌을 둔다
- 같은 브랜치를 재분석하면 그 브랜치 디렉토리 안에서만 덮어쓴다(다른 브랜치 디렉토리는 그대로 남아 개발 중 브랜치 간 비교에 쓸 수 있다)
- 릴리스 브랜치(`release/*` 등)도 특별 취급 없이 자기 브랜치 디렉토리 하나를 가질 뿐이다
- 과거 시점이 필요하면 그 시점 코드(git)를 다시 분석해 재현한다. 오래된/죽은 브랜치 디렉토리 정리는 사용자 몫이다

## 구조

- `overview.md` -- 진입점. 실행 순서 정의
- `languages/<언어>.md` -- 언어 고유 규칙(노드 단위·한정자 어휘 등). 범용 에이전트가 메인이 판정한 언어의 파일을 로드해 적용. **언어 추가 = 규칙 파일 하나**(+ 필요 시 `callgraph-<언어>` 에이전트). 현 구현 `languages/cpp.md`
- 플러그인 루트 `agents/` 의 code-analyze 에이전트들. 플러그인이 자동 발견해 Task 의 `subagent_type`(bare 이름)으로 호출된다
  - 골격(Stage 0, 조건부 위임): `code-analyze-survey`
  - 구조(Stage 1, 서브트리-major): `code-analyze-structure`
  - 가로지르는(Stage 1): `code-analyze-{architecture,flow,data-contract,test,externals,invariants}`
  - 렌더: `code-analyze-{render-html,render-markdown}`
  - 검증: `code-analyze-verify`
  - 조건부(언어별 호출그래프 슬롯): `code-analyze-callgraph-<언어>` -- 현 구현 cpp(C++ + compile_commands.json 있을 때만). 새 언어는 온디맨드로 추가

## 동작 흐름 (Stage 0~4)

**사전 -- 분석 대상 선택:** 레포 전체가 아니라 **한 응집 단위(디렉토리 하나)**를 대상으로 삼는다(독립 역할디렉토리·서브모듈을 한 번에 섞지 않는다). 시작 시 어느 디렉토리인지 묻고(README 등 구조 힌트가 있으면 참고, 없으면 물어보고, 그래도 없으면 직접 지정), **실제 존재하는 디렉토리**라야 진행한다. 산출 `analyze/` 는 그 대상 디렉토리 아래.

**사전 -- 실행 범위 게이트:** 착수 전 사용자와 시작·종료 스테이지를 정한다. facet 이 베이스라 Stage 2 에서 멈춰 facet 만 만들 수도 있고(render 옵트인), 중간 시작(1/2)은 선행 데이터(skeleton/facet)가 디스크에 있을 때만 허용한다. render 로부터 재시작(Stage 3/4 시작)은 render 재설계 뒤 로드맵 예정.

0. **골격/인벤토리(직렬, 1회)** -- 파일 트리(소스 미러) + 파일별 인벤토리(타입/함수 이름·개수) + 진입 표면 + 스키마/테스트 존재를 `facet/skeleton.md` 로. 카운팅 규율을 여기서 1회 적용해 "공유 사실"의 단일 출처를 세운다. 기본 메인 직접, 대형이면 `survey` 위임. C++이면 `callgraph-cpp` 로 호출/참조맵을 골격에 미리 채움
1. **facet 채우기(병렬)** -- 골격을 입력으로 구조(서브트리-major) + 가로지르는 6종을 Task 병렬 호출. 각 에이전트는 골격 위에 **깊이만** 더한다(개수 재계산·서론 재도출 없음). 구조 노드 facet 은 구조 에이전트가 직접 Write(부피), 가로지르는 6종은 본문 반환 -> 메인 저장(Stage 2 조인 입력)
2. **인덱스/점검/조인(메인)** -- 얇은 `index.md` 작성 + facet 링크 무결성 점검(link-lint, render 선결). 교차 facet 조인(예: invariants × test -> 미커버 불변식 공백)은 여기서
3. **렌더(병렬)** -- `render-html` / `render-markdown` 병렬 호출, `html/`·`markdown/` 저장 후 `.tmp/` 삭제
4. **검증 + 결과 안내** -- `verify` 로 HTML 다이어그램 깨짐 + 내부 링크 무결성(끊긴 상대 href) 확인, 생성 파일·경로·검증 요약 + verify 보증범위(형식만 검증, 의미 정합은 미검증) 안내

## 주의

- 작업(분석·렌더)은 에이전트에 위임한다. 가로지르는 facet 저장은 메인이 하고(본문 반환), 구조 노드 facet 과 렌더 산출물(html/markdown)은 부피/잘림 때문에 에이전트가 직접 Write 한다.
- 스킬은 생성만 한다. 직접 커밋·푸시하거나 GitHub Pages 디렉토리에 파일을 밀어넣지 않는다.
