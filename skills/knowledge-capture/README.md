...
- `overview.md` -- 진입점. 활성 조건, 두 트리거, 점수표, writer 위임 전부
...
켜진 행 점수 합계가 **3점 이상**(임계 3~4)이면 즉시 위임, 미만은 무시. 점수는 메인 LLM 컨텍스트에만 존재하며 세션 단위로 휘발(파일 저장 안 함).
...
- **writer 입력 포맷** -- `save_path` / `topic` / `triggered_by`(점수표 코드 또는 `user-request`) / `trigger_summary` / `user_known` / `discussion_context` / `snippets` / `kind` / `language` (라벨+콜론 구조화 텍스트, JSON 아님)
