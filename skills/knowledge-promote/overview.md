...
단, 콤마가 단순 연결어로 쓰였거나 "A다, 그래서 B다"처럼 한 논지가 이어지는 서술은 쪼개지 않는다. 나열·단계·독립된 사실의 핵심일 때만 목록화한다. 전부 쪼개면 글의 흐름이 끊긴다.
...
"흔한 오해 / 통념 정정"은 그 기능에 널리 퍼진 잘못된 통념이 있을 때만 두는 선택적 슬롯이다(없으면 생략). 포맷은 위 `##### 대조·반박 항목` 의 blockquote 형식(`> **오해:**` ...)을 그대로 재사용하고, code 전용 포맷을 새로 만들지 않는다. 이 슬롯은 통념 정정이 핵심인 code 노트에서 writer 가 자리 없이 임의 절을 만들지 않도록 정해 둔 것이다.
...
프로젝트 특유 요소는 산문에만 있지 않다. 코드 예제·발췌(특히 code-scan 유래 wip)에 스캔 원본의 프로젝트 고유 식별자(함수·타입·변수명)가 남아 있으면 범용 이름으로 치환한다. writer 가 원천에서 걸러도 최종 일반화는 이 단계 책임이다.
