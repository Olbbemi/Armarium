...
이 언어로 스캔할 때 훑는 파일 확장자다. 범위 집계와 순회 모두 이 목록에 걸리는 파일만 본다. 목록 밖(빌드 산출물, 프로젝트 설정, 다른 언어 소스)은 세지도 읽지도 않는다.
...
신호는 그 토픽을 확실히 대표하는 **강앵커**로 적는다. `&&`(논리 AND 와 구분 불가) · `|`(비트 OR) · 맨 `auto` 처럼 흔하고 모호한 토큰은 오탐만 늘리므로 신호로 쓰지 않는다. 같은 토픽을 대표하는 더 고유한 토큰(`std::move`, `std::ranges::`, `auto [` 등)이 있으면 그것만 신호로 둔다. 고유 강앵커가 없는 항목은 문맥 확증에 기대므로 항목마다 `recall 제한적` 으로 표시한다.
...
- `move-semantics` -- 강앵커 `std::move(`. 소유권 이전과 복사 회피.
- `perfect-forwarding` -- 강앵커 `std::forward`. 인자 완전 전달.
...
- `rule-of-zero-three-five` -- 특수 멤버(복사/이동 생성·대입, 소멸자)의 `=default`/`=delete` 조합. recall 제한적. 값 시맨틱 설계.
...
- `ranges` -- 강앵커 `std::ranges::` / `views::`(`views::filter`/`transform` 등). 지연 평가 시퀀스 변환.
