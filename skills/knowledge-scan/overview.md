...
wip 저장 경로를 확보하는 단계에서 이 파일을 Read 툴로 로드해 따른다. 본 스킬이 받는 경로는 wip 저장 디렉토리 하나이며, 공용 파일 1절의 검증 절차를 그 경로에 적용한다.
...
저장 경로를 범위 확인보다 뒤에 두는 이유는 3단계에 중단 갈래가 있기 때문이다. 스캔할 언어가 없어 물러나는 세션에까지 Herbarium 경로를 요구하지 않는다.
...
상대경로로 받았으면 절대경로로 바꿔 둔다. 스캐너는 절대경로를 입력으로 받으며, 서브에이전트의 작업 디렉토리가 메인과 같다는 보장이 없다.
...
확장자 필터를 먼저 거는 이유는 표에 실릴 것을 코드로 한정하기 위해서다. 빌드 산출물이나 다른 언어 소스는 세지 않으므로 사용자가 볼 목록이 짧아진다.
...
- 언어가 여럿이면 언어별로 나눠 제시한다. 같은 topic 명칭이 언어마다 있을 수 있어(`coroutines` 등) 섞으면 어느 언어의 것인지 가려지지 않고, wip 파일명도 겹친다.
...
위임한 topic 수만큼 완료 통지가 도착한다. 통지마다 반환 본문을 공용 프로토콜 3절대로 저장한다.

writer 는 위임 시점의 디렉토리를 보고 파일명을 정하므로, 병렬로 돌린 위임들이 같은 파일명을 권장할 수 있다. 언어가 여럿이고 같은 topic 명칭이 언어마다 있을 때 특히 그렇다. 저장 직전에 이번 실행에서 이미 저장한 파일명과 겹치는지 보고, 겹치면 뒤에 번호를 더해(`<이름>-2.md`) 저장한다.
